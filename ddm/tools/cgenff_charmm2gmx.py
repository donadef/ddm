            ...
            if line.find('!'):
                ...
                if line.startswith("ATOM"):
                    ...
                    self.G.add_node(self.natoms,
                                    type=atm[self.natoms]['type'],
                                    resname=atm[self.natoms]['resname'],
                                    name=atm[self.natoms]['name'],
                                    charge=atm[self.natoms]['charge'],
                                    mass=atm[self.natoms]['mass'],
                                    beta=atm[self.natoms]['beta'],
                                    x=atm[self.natoms]['x'],
                                    y=atm[self.natoms]['y'],
                                    z=atm[self.natoms]['z'],
                                    segid=atm[self.natoms]['segid'],
                                    resid=atm[self.natoms]['resid'])
    ...
    def write_pdb(self, f):
        for atomi in range(0, self.natoms):
            if len(self.G.node[atomi]['name']) > 4:
                ...
                exit()
            elif len(self.G.node[atomi]['name']) == 4:
                f.write("%-6s%5d %-4s %-4s%5s%12.3f%8.3f%8.3f%6.2f%6.2f\n" %
                        ("ATOM", atomi + 1, self.G.node[atomi]['name'], self.name,
                         self.G.node[atomi]['resid'], self.coord[atomi][0],
                         self.coord[atomi][1], self.coord[atomi][2], 1.0,
                         self.G.node[atomi]['beta']))
            else:
                f.write("%-6s%5d  %-3s %-4s%5s%12.3f%8.3f%8.3f%6.2f%6.2f\n" %
                        ("ATOM", atomi+1, self.G.node[atomi]['name'], self.name,
                         self.G.node[atomi]['resid'], self.coord[atomi][0],
                         self.coord[atomi][1], self.coord[atomi][2], 1.0,
                         self.G.node[atomi]['beta']))
...
def main(mol_name, mol2_name, rtp_name, ffdir):
...
if __name__ == '__main__':
    ...
    print("NOTE1: Code tested with python 2.7.3. Modified to be compatible with python 3. Your python version:", sys.version)
    print("")
    ...
    main(mol_name, mol2_name, rtp_name, ffdir)

    print("============ DONE ============")
    print("Conversion complete.")
    print(f"The molecule topology has been written to {mol_name.lower()}.itp")
    print(f"Additional parameters needed by the molecule are written to {mol_name.lower()}.prm, which needs to be included in the system .top")
    print("============ DONE ============")
